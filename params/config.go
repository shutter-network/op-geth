...
type ChainConfig struct {
	...
	MeadowTime   *uint64  `json:"meadowTime,omitempty"`   // Meadow switch time (nil = no fork, 0 = already on optimism meadow)
...
func (c *ChainConfig) IsMeadow(time uint64) bool {
	return isTimestampForked(c.MeadowTime, time)
}
...
func (c *ChainConfig) IsOptimismMeadow(time uint64) bool {
	return c.IsOptimism() && c.IsMeadow(time)
}
...
func (c *ChainConfig) Rules(num *big.Int, isMerge bool, timestamp uint64) Rules {
	...
	return Rules{
		...
		IsOptimismMeadow:   c.IsOptimismMeadow(timestamp),
	}
}
